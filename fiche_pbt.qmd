...
Il faut compter entre 15 et 30 ceps par heure et par personne selon le cépage, le type de taille et la présence de palissage (le tirage des bois complique la tâche).

Il est possible de faire une pesée par groupe de souche (placette) pour gagner du temps, et atteindre 40 à 50 ceps par heure et par personne. Si la variabilité entre ceps est faible, la perte de puissance statistique reste acceptable. Dans le cas contraire, il est préférable de faire des pesées cep par cep.
...
[^1]: Les noms des variables dans le référentiel de la vitis ontology @duchene2019 sont indiqués en gras avec entre parenthèse l'identifiant unique de la variable.

### Expression par cep

Les variables suivantes peuvent être définies :

-   le poids total des sarments par cep, exprimé en kg/cep : **PRUN_FW_PLANT** (CO_356:1000190)

-   le nombre total de sarments par cep : **NB_CANES_PLANT** (CO_356:1000157).
...
-   Le poids de bois de taille peut être exprimé en kg/m² de sol en prenant en compte la densité de plantation : **PRUN_FW_M2** (CO_356:1000290).

$$ PRUN\_FW\_M2 = PRUN\_FW\_PLANT * (densité_{ha} /10 000) $$

-   Le nombre de sarments peut lui aussi être exprimé par m² de sol (NB_CANES_M2) en prenant en compte la densité de plantation.
